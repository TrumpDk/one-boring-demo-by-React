...
class Goods extends React.Component {
  ...
  async fetchData(id) {
    try {
      ...
      const { goodsList } = await HttpService.getGoodsRelated({ id });
      let {
        info,
        gallery,
        brand,
  ...
  async fetchCartNum() {
    try {
      const { errno, errmsg, cartTotal } = await HttpService.getCartNum({});
      if (errno === 401) {
  ...
  changeBuyNum(val, stock) {
    if (
      ...
      this.state.goodsBuyNum + val <= stock
    ) {
      this.setState({
  ...
  changeSizeActiveId(index, item) {
    ...
    if (specificationList[index].activeId !== item.id) {
      ...
      const { stock } = this.computedGoodsSizePriceAndSizeName(
        specificationList,
        productList
      );
      this.setState({
  ...
  async addCart(priceId, goodsBuyNum, isShowSize) {
    ...
    const { errno, errmsg } = await HttpService.postAddCart({
  ...
  render() {
    const {
      info: { goodsName, subTitle, goods_desc },
      brand,
      gallery,
      isShowSize,
      comment,
    ...
    const {
      ...
      stock,
      priceId
    } = this.computedGoodsSizePriceAndSizeName(specificationList, productList);
    const {
      history: { push },
      isloading
    } = this.props;
      ...
      <div className="goodsPage">
        ...
        >
          <div className="goodsSizeDo">
            <div className="goodsSizeSetMsg">
              {gallery.length ? (
                <img src={gallery[0].img_url} alt="goods" />
              ) : null}
                  ...
                  <div className="goodsSizeListWrap">
                    {item.valueList.map(temp => (
                      <div
                        ...
                        onClick={this.changeSizeActiveId.bind(
                          this,
                          index,
                          temp,
                          stock
                  ...
                  <div className="goodsSizeListWrap">
                    <div className="goodsBuyCount">
                      <div
                        ...
                        onClick={this.changeBuyNum.bind(this, -1, stock)}
                      >
                        -
                      </div>
        ...
        {comment.count ? (
          <div className="goodsComment">
            ...
            {'data' in comment ? (
              <CommentList commentList={[comment.data]}></CommentList>
            ) : null}
          </div>
        ) : null}
        ...
        <div className="goodsPageDo">
          <div
            className={userHasCollect === 1 ? 'isLike like' : 'isLike'}
          ...
          <div
            className="addCart"
            onClick={this.addCart.bind(this, priceId, goodsBuyNum, isShowSize)}
          >
            加入购物车
          </div>
          <div className="payGoods" onClick={this.payGoods.bind(this)}>
            立即购买
          </div>
        </div>
      </div>
